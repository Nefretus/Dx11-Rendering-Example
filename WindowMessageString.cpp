...
WindowMessageString::WindowMessageString()
	:
	map({
...
void WindowMessageString::PrintWindowMessage(UINT msg) {
	auto it = map.find(msg);
	if (it != map.end()) {
		OutputDebugString(map[msg]);
		OutputDebugString(L"\n");
	}
	else {
		OutputDebugString(L"Unknown message!");
	}
}
